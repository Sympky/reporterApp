        ...
        try {
            ...
            if ($hasBlockMethodologies) {
                ...
                foreach ($methodologies as $index => $methodology) {
                    ...
                    $replacement = [
                        'methodology_title' => $methodology->name ?? 'Untitled Methodology',
                        'methodology_description' => $methodology->description ?? 'No description provided'
                    ];
                ...
                try {
                    ...
                    foreach ($methodologies as $methodology) {
                        $methodologiesText .= "## " . ($methodology->name ?? 'Untitled Methodology') . "\n\n";
                        $methodologiesText .= ($methodology->description ?? 'No description provided.') . "\n\n";
                    }
            ...
            elseif (in_array('methodology_title', $variables) || in_array('methodology_name', $variables)) {
                ...
                if (count($methodologyPlaceholders) > 1) {
                    ...
                    foreach ($methodologyPlaceholders as $index => $placeholder) {
                        ...
                        if (isset($methodologies[$methodologyIndex])) {
                            ...
                            $templateProcessor->setValue($field . $indexSuffix, $methodology->name ?? 'Untitled Methodology');
                            ...
                            if (in_array($descField . $indexSuffix, $variables)) {
                                $templateProcessor->setValue($descField . $indexSuffix, $methodology->description ?? 'No description provided');
                            }
                ...
                // Handle single placeholder - concatenate all methodologies 
                    ...
                    $allMethodologies = '';
                    foreach ($methodologies as $methodology) {
                        $allMethodologies .= "## " . ($methodology->name ?? 'Untitled Methodology') . "\n\n";
                        $allMethodologies .= ($methodology->description ?? 'No description provided.') . "\n\n";
                    }
